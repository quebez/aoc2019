...
let first = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr[i].length; j++) {
      if (arr[i][j] == '#') {
        res.push(getAsteroidsDetected(arr, i, j));
      }
    }
  }
...
let getAsteroidsDetected = (arr, y, x) => {
  ...
  for (let yi = 0; yi < yLength; yi++) {
    for (let xi = 0; xi < xLength; xi++) {
      if (arr[yi][xi] == '#') {
        results.push(Math.atan2(yi - y, xi - x));
      }
    }
  }
...
/************* SECOND TASK *************/
let second = (arr) => {
  let res = destroyAsteroids(arr, 17, 13);
  console.log(res);
}

/* Helpers for 2nd task */
let destroyAsteroids = (arr, y, x) => {
  let xLength = arr[0].length;
  let yLength = arr.length;
  let results = {};
  ...
  for (let yi = 0; yi < yLength; yi++) {
    for (let xi = 0; xi < xLength; xi++) {
      if (arr[yi][xi] == '#') {
        let coo = [yi - y, xi - x];
        let actCoo = [yi, xi];
        let rad = parseFloat(Math.atan2(...coo) * 180 / Math.PI) + 90;
        if (rad < 0) rad = 360 + rad;
        if (results[rad] == undefined) results[rad] = [actCoo];
        else results[rad].push(actCoo);
      }
    }
  }
  ...
  for (let i = 0; i < 200; i++) {
    ...
    Object.keys(results).forEach(key => {
      if (parseFloat(key) < next && parseFloat(key) > previous) next = key;
    });

    //console.log(i, next, results[next]);
  }
...
/*
Text:
Advent of Code[About][Events][Shop][Settings][Log Out]quebez 20*
