...
/************* FIRST TASK *************/
let first = (arr) => {
  let first = getPaths(arr[0]);
  let second = getPaths(arr[1]);
  ...
  for (const f in first) {
    ...
    for (let i = 0; i < first[f].length; i++) {
      for (let j = 0; j < second[f].length; j++) {
        if (first[f][i] == second[f][j]) crossings.push([f, first[f][i]]);
      }
    }
  }
...
let getPaths = (vectors) => {
  let lastLoc = [0,0];
  let obj = { 0: [0] };

  for (let i = 0; i < vectors.length; i++) {
    lastLoc = getPath(vectors[i], lastLoc, obj);
  }
...
let getPath = (vector, lastLoc, obj) => {
  ...
  for (let i = 0; i < path; i++) {
    ...
    obj[lastLoc[0]] ? obj[lastLoc[0]].push(lastLoc[1]) : obj[lastLoc[0]] = [lastLoc[1]];
  }
  return lastLoc;
}
